...
async function init() {
  ...
  // core / mini / tonal / draw の全エクスポートを評価スコープ(globalThis)に注入する
  // draw を含めることで、レッスンが使う `._punchcard()` 等の可視化メソッドが
  // headless でも解決でき(描画は行われないが query は通る)、スモークが落ちない。
  await evalScope(
    import('@strudel/core'),
    import('@strudel/mini'),
  ...
  // 可視化メソッドの underscore 版(_punchcard=コード内インライン描画 等)は最新の
  // strudel.cc にはあるが、ピン留めした @strudel/draw 1.2.6 には未登録。headless では
  // 描画しないので、未定義なら pass-through(this をそのまま返す)シムを置き、
  // レッスンが `._punchcard()` を使っていてもスモーク評価が落ちないようにする。
  const { Pattern } = await import('@strudel/core');
  for (const name of ['_punchcard', '_pianoroll', '_spiral', '_scope']) {
    if (typeof Pattern.prototype[name] !== 'function') {
      Pattern.prototype[name] = function () {
        return this;
      };
    }
  }
